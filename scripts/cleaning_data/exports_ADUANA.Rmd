...
This document cleans the Aduana exports datasets and maatches the item_sa with the databease we built with aduna codes for each of the species on the SERNAPESCA datasets. 
...
cod_aduana_sp <- read_excel(here("information/raw_databases/ADUANAS_CHILE/arancel_aduanero_nomenclatura/arancel_aduanero_2017/codigos_aduana_por_especie_2017.xlsx"), sheet = "Hoja1", range = "A1:G701") %>%
...
match_exports <- exports_yr %>% 
  ...
  group_by(year, region, especie_SERNAPESCA) %>% 
  summarise(tons = sum(tons, na.rm = T)) %>% 
  dplyr::mutate(especie_SERNAPESCA = ifelse(stringr::str_detect(especie_SERNAPESCA,"SARDINA ESPA.*OLA"), "SARDINA ESPANOLA", especie_SERNAPESCA)) ##correct name
