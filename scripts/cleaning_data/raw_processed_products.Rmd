...
#save
write.csv(raw_no_HC, here("clean_databases/raw_NHC.csv"), row.names = F)
...
NHC_m_product_type <- raw_no_HC %>% 
  left_join(sp_index, by="species")

##Save!!!!!!
write.csv(NHC_m_product_type, here("clean_databases/NHC_m_product_type.csv"), row.names = F)
...
NHC_sin_algas <- raw_no_HC %>% 
  filter(product %in% c("fish meal", "oil")) %>% 
  left_join(sp_index, by="species") %>% 
  group_by(year, region,sc_name) %>% 
  summarise(total_NHC= sum(tons, na.rm = T)) %>% 
  filter(!(sc_name == 'NA'))
...
##Save!!!!!!
write.csv(NHC_sin_algas, here("clean_databases/NHC_sin_algas.csv"), row.names = F)

```


NO HUMAN CONSUMPTION PRODUCT

This will be used for substracting all algae products from export in order to avoid double accounting.


```{r}

harina_2013_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2013.xlsx"), sheet = "Harinareg", range = "A6:Q44") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>% 
  gather(key = region, value = tons, 2:16)%>% 
...
harina_2014_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2014.xlsx"), sheet = "Harinareg", range = "A6:Q54") %>% 
   rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>% 
  gather(key = region, value = tons, 2:16)%>% 
...
harina_2015_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2015.xlsx"), sheet = "Harinareg", range = "A6:Q46") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>% 
  gather(key = region, value = tons, 2:16)%>% 
...
harina_2016_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2016.xlsx"), sheet = "Harinareg", range = "A6:Q44") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
harina_2017_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2017.xlsx"), sheet = "Harinareg", range = "A6:Q46") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
agar_2013_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2013.xlsx"), sheet = "Agarreg", range = "A6:Q8") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
agar_2014_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2014.xlsx"), sheet = "Agarreg", range = "A6:Q10") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
agar_2015_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2015.xlsx"), sheet = "Agarreg", range = "A6:Q8") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
agar_2016_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2016.xlsx"), sheet = "Agarreg", range = "A6:Q8") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
agar_2017_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2017.xlsx"), sheet = "Agarreg", range = "A6:Q8") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
carragenina_2013_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2013.xlsx"), sheet = "Carrageninareg", range = "A6:Q12") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
carragenina_2015_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2015.xlsx"), sheet = "Carrageninareg", range = "A6:Q12") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
carragenina_2016_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2016.xlsx"), sheet = "Carrageninareg", range = "A6:Q12") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
carragenina_2017_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2017.xlsx"), sheet = "Carrageninareg", range = "A6:Q14") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
colagar_2013_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2013.xlsx"), sheet = "Colagarreg", range = "A6:Q8") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
colagar_2014_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2014.xlsx"), sheet = "Colagarreg", range = "A6:Q8") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>% 
  gather(key = region, value = tons, 2:16)%>% 
...
colagar_2015_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2015.xlsx"), sheet = "Colagarreg", range = "A6:Q10") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>% 
  gather(key = region, value = tons, 2:16)%>% 
...
colagar_2016_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2016.xlsx"), sheet = "Colagarreg", range = "A6:Q8") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>% 
  gather(key = region, value = tons, 2:16)%>% 
...
colagar_2017_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2017.xlsx"), sheet = "Colagarreg", range = "A6:Q8") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>% 
  gather(key = region, value = tons, 2:16)%>% 
...
aceite_2013_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2013.xlsx"), sheet = "Aceitereg", range = "A6:Q31") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
aceite_2014_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2014.xlsx"), sheet = "Aceitereg", range = "A6:Q42") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
aceite_2016_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2016.xlsx"), sheet = "Aceitereg", range = "A6:P31") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:15)%>% 
...
aceite_2017_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2017.xlsx"), sheet = "Aceitereg", range = "A6:Q34") %>%
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
alginato_2015_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2015.xlsx"), sheet = "Alginatoreg", range = "A6:Q8") %>% 
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
##Alga seca
algaseca_2013_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2013.xlsx"), sheet = "ALGASECAespreg", range = "A6:Q32") %>%
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
algaseca_2014_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2014.xlsx"), sheet = "Algaseca_region", range = "A6:Q28") %>%
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
algaseca_2015_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2015.xlsx"), sheet = "ALGA SECA_REGION", range = "A6:Q30") %>%
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
algaseca_2016_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2016.xlsx"), sheet = "ALGA SECA_REGION", range = "A6:Q34") %>%
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
algaseca_2017_p <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2017.xlsx"), sheet = "ALGA SECA_REGION", range = "A6:Q36") %>%
  rename(m_p= X__1) %>% 
  filter(m_p == "P") %>% 
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
...
##combine all NHC producs
product_no_HC <- dplyr::bind_rows(aceite_2013_p, aceite_2014_p, aceite_2016_p, agar_2013_p, agar_2014_p, agar_2015_p, agar_2016_p, alginato_2015_p, carragenina_2013_p, carragenina_2015_p, carragenina_2016_p, colagar_2013_p, colagar_2014_p, colagar_2015_p, colagar_2016_p, harina_2013_p, harina_2014_p, harina_2015_p, harina_2016_p, harina_2017_p,algaseca_2013_p,algaseca_2014_p ,algaseca_2015_p, algaseca_2016_p , algaseca_2017_p) %>% 
  filter(!(species == 'NA')) %>% 
  mutate(tons = as.numeric(tons)) %>% 
  left_join(species_index, by = "species")

##Ssave
write.csv(product_no_HC, here("clean_databases/product_NHC.csv"), row.names = F)


##Save
# write.csv(product_algae_no_HC, here("clean_databases/product_algae_NHC.csv"), row.names = F)
...
##Save
write.csv(product_HC, here("clean_databases/products_HC.csv"), row.names = F)
...
congelado_2017_m <- read_excel(here("information/raw_databases/SERNAPESCA/products/products_per_species_region_month_2017.xlsx"), sheet = "CONG_REGION", range = "A6:Q172") %>%
  ...
  select(-m_p)%>%
  gather(key = region, value = tons, 2:16)%>% 
  mutate (product = 'frozen', year = 2017, tons = ifelse(tons == "-", 0, tons)) %>% 
  rename(species =  ESPECIE)
