...
** Main fisheries in terms of volume**

According to landing records from SERNAPESCA over the last five years, the artisanal sector lands on average 1.6 million tons per year of marine species while the industrial sector lands around 0.9 million tons per year. The most landed species by the artisanal sector have been the Araucanian herring (*Strangomera bentincki*), the Giant grey weed (*Lessonia nigrescens*) and the Peruvian anchoveta (*Engraulis ringens*). Although 

The most landed species by the industrial sector are the Peruvian anchoveta (*Engraulis ringens*), the Chilean jack mackerel (*Trachurus murphyi*) and the Araucanian herring (*Strangomera bentincki*).

Both sectors present no major changes in catch composition for their main species. In the case of artisanal catches 
...
annual_landings_sector <- landings %>% 
  ...
  group_by(subsector) %>% 
  summarise(annual_tons = mean(tons))
...
# Top 5 landed per sector
 ...
 landings_top_5 <- landings %>% 
  ...
  ggtheme_plot()
   ...
   ## Top 5 landed species per region artisanal sector

 landings_reg_ind <- landings %>% 
  ...
  ## Top 5 landed species per region industrial sector
...
# Top aquaculture
  ...
  top_n(5, tons) %>% 
  left_join(species_list) %>% 
  ggplot(aes(x = year, y = tons, fill = sc_name))+
  geom_bar(stat = 'identity') +
 ...
 ** Main Traded fisheries **
 Imports
 Exports
 Net trade
 
 ** Regional Diferences in Landings **
