...
OJO: HAY QU VER COMO AGREGAR EL NOMBRE CIENTIFICO!! Probablemnte modificar bases de datos desde el principio en otros scripts.
...
###FATLTA AGREGAR NOMBRES COMUN EN INGLES!!!
en_common <- common_names(sp_list$sc_name, server = NULL, Language = "English",
fields = NULL) %>% 
  filter(Language == "English")
## las lista wu genera fishbase no ingluye todas las especies de sp_list
...
database1_complete <- landings %>% 
  full_join(aquaculture, by= c("year", "region", "species", "variable", "sc_name")) %>%
...
setdiff(database1_complete$sc_name, DHC_yield$sc_name)
setdiff(DHC_yield$sc_name, database1_complete$sc_name)
