...
exports <- read_csv(here("clean_databases/exports_ADUANA.csv")) %>% 
  select(year, species= especie_SERNAPESCA, exports= tons) %>%
  filter(year!= 2018) %>% 
  mutate(species = str_to_upper(species)) %>%
  left_join(sp_index, by= "species") %>% 
  select(year, sc_name, exports) %>% 
  group_by(year, sc_name) %>% 
...
DHC_yield <- read_xlsx(here("clean_databases/intermediate/sp_cl_DHC.xlsx")) %>% 
  select(sc_name, mean_yield)



intermediate_df <- landings_an %>% 
  left_join(aquaculture, by= c("year","sc_name")) %>% 
  left_join(non_human_cons, by= c("year","sc_name")) %>%
  left_join(exports, by= c("year","sc_name")) %>% 
  left_join(DHC_yield, by= "sc_name") %>% 
  unique()
  
intermediate_df$export[is.na(intermediate_df$export)] <- 0
intermediate_df$total_NHC[is.na(intermediate_df$total_NHC)] <- 0
intermediate_df$aquaculture[is.na(intermediate_df$aquaculture)] <- 0
intermediate_df$catch[is.na(intermediate_df$catch)] <- 0
